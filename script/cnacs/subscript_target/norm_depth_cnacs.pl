...
while (<SEG_TMP>) {
	...
	my $ploidy = 'NA';
	if ( $curRow[5] ne 'NA' ) {
		$ploidy =  2 * 2 ** ( $curRow[5] - $compensation );
	}
	my $as = 'NA';
	if ( $curRow[6] ne 'NA' ) {
		$as = 2 ** $curRow[6];
	}
	next if ( ( $ploidy eq 'NA' ) || ( ( ( $ploidy < 2.2 ) && ( $ploidy > 1.8 ) ) && ( ( $as eq 'NA' ) || ( $as > 0.9 ) ) ) );
	...
	if ( $chr_num == 23 ) {
		next if ( ( $ploidy eq 'NA' ) || ( ( $ploidy > 1.8 ) && ( $ploidy < 2.2 ) ) );
		
		my $par_flag = 0;
		foreach my $idx ( 0 .. $#par_start ) {
			$par_flag = 1 if ( ( $start > $par_start[$idx] ) && ( $end <= $par_end[$idx] ) );
		}
		next if ( ( $par_flag == 0 ) && ( ( $ploidy eq 'NA' ) || ( ( $ploidy > 0.8 ) && ( $ploidy < 1.2 ) ) ) );
	}
	
	if ( $chr_num == 24 ) {
		next if ( ( $ploidy eq 'NA' ) || ( ( $ploidy > 0.8 ) && ( $ploidy < 1.2 ) ) );
		next if ( ( $ploidy eq 'NA' ) || ( ( $ploidy > 1.8 ) && ( $ploidy < 2.2 ) ) );
	}
