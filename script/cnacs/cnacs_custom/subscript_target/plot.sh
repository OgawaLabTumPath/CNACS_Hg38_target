...
readonly OUTPUT_ALL=${OUTPUTDIR}/${ID}/${ID}_all.png
...
echo "${R_PATH} --vanilla --slave --args ${INPUT}.tmp ${SEGMENT}.tmp ${CENTROMERE} ${UPPER_LOD} ${LOWER_LOD} ${OUTPUT_ALL} < ${COMMAND_CNACS_CUSTOM}/subscript_target/plot_all.R"
${R_PATH} --vanilla --slave --args ${INPUT}.tmp ${SEGMENT}.tmp ${CENTROMERE} ${UPPER_LOD} ${LOWER_LOD} ${OUTPUT_ALL} < ${COMMAND_CNACS_CUSTOM}/subscript_target/plot_all.R
